...
/*===================*/
	...
	function nz_single_image($atts, $content = null){
		...
		$image_string = (!empty( $link_to )) ? '<a class="nz-single-image" target="'.$link_target.'" href="' . $link_to . '"'. '>' . $img_output . '</a>' : $img_output;
		$output .= $image_string;
		return $output;
	}
		...
		if (isset($images) && !empty($images)) {
			...
			foreach ( $images as $attach_id ) {
				...
				if ( $attach_id > 0 ) {
					...
					if ($link_full == "true") {
						$gal_images .= '<div class="gallery-item element-animate">'.$before_img.'<div class="ninzio-overlay" ><a data-lightbox-gallery="gallery1" class="nz-overlay-before projects-link" href="'.$link[0].'"></a></div><img alt="'.$thumb_img->post_excerpt.'" src="'.$img[0].'" width="'.$img[1].'" height="'.$img[2].'" />'.$after_img.'</div>';
					} else {
						$gal_images .= '<div class="gallery-item element-animate">'.$before_img.'<img src="'.$img[0].'" width="'.$img[1].'" height="'.$img[2].'" />'.$after_img.'</div>';
					}
				}
			...
			$output .= '<div class="nz-gallery nz-clearfix '.sanitize_html_class($version).' '.sanitize_html_class($el_class).' animate-'.sanitize_html_class($animate).'" data-columns="'.absint($columns).'" data-autoplay="'.$autoplay.'" data-animate="'.$data_animate.'">'.$gal_images.'</div>';
			return $output;
		}
	}
...
/*===================*/
	...
	function nz_btn($atts, $content = null) {
		...
		$output    = "";
		$animation = "element-animate-true";
		...
		if ($effect == "none") {
			$animation  = "element-animate-false";
		}
		...
		$output .= '<a class="button '.$class.' '.$animation.'" href="'.esc_url($link).'" target="'.esc_attr($target).'" data-effect="'.sanitize_html_class($effect).'">';
...
/*===================*/
	...
	function nz_sep($atts, $content = null) {
		...
		$output = '<div class="sep-wrap element-animate '.$animation.' '.sanitize_html_class($align).' nz-clearfix" data-effect="'.sanitize_html_class($effect).'"><div class="nz-separator '.sanitize_html_class($type).'" style="'.esc_attr($styles).'">&nbsp;</div></div>';
		return $output;
	}
	    	...
	    	if ($modal == "true") {
	    		...
	    		switch( $tag ) {
			        case "nz_youtube":
			            ...
			            break;
			        case "nz_vimeo":
			            $output .= '<a class="video-modal" href="//vimeo.com/'.$id.'">';
	    					$output .= '<div class="ninzio-overlay"><div class="ninzio-overlay-content"><div class="video-icon"></div><h3 class="video-title">'.$title.'</h3></div></div>';
			            	$output .= '<img width="'.$img_attributes[1].'" height="'.$img_attributes[2].'" src="'.$img_attributes[0].'">';
			            $output .= '</a>';
			            break;
...
/*===================*/
	...
	function nz_soundcloud($atts) {
		...
		$params = 'color='.substr($GLOBALS['montserrat_ninzio']['main-color'], -6).'&auto_play=false&show_artwork=true';
...
/*===================*/
	...
	function nz_tweets($atts, $content = null) {
		...
		$consumer_key        = ($GLOBALS['montserrat_ninzio']['tweets-consumer-key']) ? esc_attr($GLOBALS['montserrat_ninzio']['tweets-consumer-key']) : "";
		$consumer_secret     = ($GLOBALS['montserrat_ninzio']['tweets-consumer-secret']) ? esc_attr($GLOBALS['montserrat_ninzio']['tweets-consumer-secret']) : "";
		$access_token        = ($GLOBALS['montserrat_ninzio']['tweets-access-token']) ? esc_attr($GLOBALS['montserrat_ninzio']['tweets-access-token']) : "";
		$access_token_secret = ($GLOBALS['montserrat_ninzio']['tweets-access-token-secret']) ? esc_attr($GLOBALS['montserrat_ninzio']['tweets-access-token-secret']) : "";
		...
		if (isset($icon_color) && !empty($icon_color)) {
			...
			$styles .= '#nz-tweets-'.$id_counter.':before {color:'.$GLOBALS['montserrat_ninzio']['main-color'].';}';
		}
		...
		if (!empty($consumer_key) && !empty($consumer_secret) && !empty($access_token) && !empty($access_token_secret)) {
			...
			$output .= montserrat_ninzio_get_the_widget( 'Ninzio_Addons_WP_Widget_Twitter', $instance,$args);
...
/*===================*/
	...
	function nz_mailchimp($atts, $content = null) {
		...
		if (isset($action) && !empty($action) && isset($name) && !empty($name)) {
			...
			$output .= montserrat_ninzio_get_the_widget( 'Ninzio_Addons_WP_Widget_Mailchimp', $instance,$args);
...
/*===================*/
	...
	function nz_tagline($atts, $content = null) {
		...
		$button_class  = "button-".sanitize_html_class($button_type);
		$button_class  .= " ".sanitize_html_class($button_color);
		$button_class  .= " ".sanitize_html_class($button_shape);
		if (isset($button_icon) && !empty($button_icon)) {
			$button_class  .= " icon-true";
		}
		...
		$output .= '<div id="nz-tagline-'.$id_counter.'" style="'.$styles.'" class="nz-tagline">';
			$output .= '<div class="container nz-clearfix">';
				...
				if (isset($button_link) && !empty($button_link)) {
					$output .= '<a class="button '.$button_class.' full-false medium animate-false anim-type-ghost hover-fill" href="'.esc_url($button_link).'">';
						$output .= '<span class="txt">'.esc_html($button_text).'</span>';
						if (isset($button_icon) && !empty($button_icon)) {$output .= '<span class="btn-icon '.sanitize_html_class($button_icon).'"></span>';}
					$output .= '</a>';
				}
			$output .= '</div>';
		$output .= '</div>';
...
/*===================*/
	...
	function nz_popup($atts, $content = null) {
		...
		$icon = (isset($icon) && !empty($icon)) ? esc_attr($icon) : 'icon-plus';
		...
		$output = '<span class="nz-popup '.$icon.'" data-position="'.$position.'" '.$direction.' data-width="'.$message_width.'" data-tipso="'.strip_tags($message).'" style="background-color:'.esc_attr($color).';"><span style="border-color:'.esc_attr($color).';" class="nz-popup-border"></span></span>';
			...
			switch ($type) {
				...
				case 'vimeo':
					...
					if (isset($id) && !empty($id)) {
						$output .='<div class="video-embed">';
					    	$output .='<div class="flex-mod">';
					    		$output .= '<iframe src="//player.vimeo.com/video/'.esc_attr($id).'" class="iframevideo" title="'.esc_attr($description).'"></iframe>';
					    	$output .='</div>';
					    $output .='</div>';
					}
...
/*===================*/
	...
	function nz_icon_progress($atts, $content = null) {
		...
		if(isset($active_color) && !empty($active_color)) {
			...
			$data_color = $GLOBALS['montserrat_ninzio']['main-color'];
		}
...
/*===================*/
	...
	function nz_circle($atts, $content = null) {
		...
		if(isset($bar_color) && !empty($bar_color)) {
			...
			$data_attr .= ' data-bar="'.$GLOBALS['montserrat_ninzio']['main-color'].'"';
		}
		...
		$output .= '<div class="nz-circle">';
			...
			$output .= '<div class="circle" '.$data_attr.' data-percent="'.absint($percentage).'">';
				$output .= '<span class="title" '.$color_styles.'>'.absint($percentage).'</span>';
			$output .= '</div>';
...
/*===================*/
	...
	function nz_steps($atts, $content = null) {
		...
		static $id_counter = 1;
		$output .= '<div class="nz-steps ' . sanitize_html_class($el_class) . '">';
			$output .= wpb_js_remove_wpautop( $content );
		$output .= '</div> ';
		return $output;
	}
	add_shortcode('nz_steps', 'nz_steps');

	function nz_step($atts, $content = null) {
		extract(shortcode_atts(array('title'    => '','step'    => ''), $atts));
		$output = "";

		$output .= '<div data-target="tab-'. (empty($tab_id) ? sanitize_title( $title ) : $tab_id) .'" class="step">';
			if (isset($step) && !empty($step)) {
				$output .= '<span class="step-title">'.$step.'</span>';
			}
			if (isset($title) && !empty($title)) {
		...
		$output .= '<div id="step-'. (empty($tab_id) ? sanitize_title( $title ) : $tab_id) .'" class="step-content">';
			$output .= ($content=='' || $content==' ') ? esc_html__("Empty step. Edit page to add content here.", 'ninzio_addons') : wpb_js_remove_wpautop($content);
		$output .= "\n\t\t\t" . '</div> ';
		return $output;
	}
	add_shortcode('nz_step', 'nz_step');
        ...
        $output .= '<div class="nz-count" id="nz-count-'.$id_counter.'">';
        	$output .= '<div class="nz-count-wrap nz-clearfix">';
				...
				if (isset($border_color) && !empty($border_color)) {
					...
					$output .= '<span class="count-border" '.$border_color.'></span>';
				}
			$output .= '</div>';
        $output .= '</div>';
...
/*===================*/
	...
	function nz_content_box($atts, $content = null) {
		...
		static $id_counter = 1;
		$output = '<div class="nz-content-box nz-clearfix '.sanitize_html_class($version).' '.sanitize_html_class($align).' animate-'.sanitize_html_class($animate).'" data-columns="'.esc_attr($columns).'" data-animate="'.$data_animate.'">'.do_shortcode($content).'</div>';
		$id_counter++;
		return $output;
	}
	...
	function nz_box($atts, $content = null) {
		...
		$output .= '<div id="nz-box-'.$id_counter.'" class=" '.$extra_class.' nz-box element-animate">';
			$output .= '<div class="nz-box-wrap">';
				$output .= '<style>';
					...
					if(isset($icon_border_color) && !empty($icon_border_color)){
						$output .= '#nz-box-'.$id_counter.' .box-icon-wrap {box-shadow:inset 0 0 0 2px '.$icon_border_color.';}';
					}
					if (isset($icon_hover_color) && !empty($icon_hover_color)) {
						$output .= '#nz-box-'.$id_counter.':hover .box-icon {color:'.$icon_hover_color.' !important;}';
					}
					if (isset($icon_hover_back_color) && !empty($icon_hover_back_color)) {
						$output .= '#nz-box-'.$id_counter.' .box-icon-wrap:before {background-color:'.$icon_hover_back_color.';}';
					}
				$output .= '</style>';
				if(isset($icon) && !empty($icon)){
				...
				$output .= '<div class="box-data">';
					if(isset($title) && !empty($title)){
						$output .='<h3 class="box-title">'.$title.'</h3>';
					}
					$output .= do_shortcode($content);
					if (isset($link) && !empty($link)) {
						$output .='<div class="nz-clearfix"><a class="box-more" href="'.esc_url($link).'">'.esc_html__('Read more', 'ninzio-addons').'</a></div>';
					}
				$output .= '</div>';
			$output .= '</div>';
		$output .= '</div>';
...
/*===================*/
	...
	function nz_testimonials($atts, $content = null) {
		...
		$output = "";
		$output .= '<div id="nz-testimonials-'.$id_counter.'" data-autoplay="'.esc_attr($autoplay).'" data-columns="3" class="'.sanitize_html_class($animate).' nz-testimonials lazy">';
			$output .= do_shortcode($content);
		$output .= '</div>';
	...
	function nz_testimonial($atts, $content = null) {
		...
		$output .= '<div class="testimonial">';
			$output .= '<div class="test-wrap nz-clearfix">';
...
/*===================*/
	...
	function nz_tab($atts, $content = null) {
		...
		$output .= '<div data-target="tab-'. (empty($tab_id) ? sanitize_title( $title ) : $tab_id) .'" class="tab '.$active.'">'.$icon.$title.'</div>';
...
/*===================*/
	...
	function nz_toggle($atts, $content = null) {
		...
		$output .= '<div class="'.sanitize_html_class($open).' toggle-title nz-clearfix">';
			$output .= '<span class="toggle-icon"></span>'.$title;
		$output .= '</div> ';
			...
			$output .= '<div class="animate-item person '.$classes.'">';
				$output .='<div class="img">';
					...
					$output .= '<div class="ninzio-overlay">';
							$output .= '<div class="ninzio-overlay-content">';
				...
				$output .= '<div class="person-body">';
					$output .='<div class="person-meta">';
						...
						if(isset($title) && !empty($title)){
							$output .= '<div class="title">'.esc_html($title).'</div><div class="nz-clearfix"></div>';
						}
					$output .= '</div>';
				$output .= '</div>';
			$output .= '</div>';
			...
			switch ($type) {
				case 'youtube':
					...
					if (isset($id) && !empty($id)) {
						$output .='<div class="video-embed">';
					    	$output .='<div class="flex-mod">';
					    		$output .= '<iframe src="//www.youtube-nocookie.com/embed/'.esc_attr($id).'" class="iframevideo" title="'.esc_attr($description).'"></iframe>';
					    	$output .='</div>';
					    $output .='</div>';
					}
					break;
...
/*===================*/
	...
	function nz_column($atts, $content = null) {
		...
		extract(shortcode_atts(
			array(
				'high'        => 'false',
				'color'   	  => $GLOBALS['montserrat_ninzio']['main-color'],
				'price'       => '29.99',
				'currency'    => '$',
				'tariff'      => '/m',
				'title'       => 'Column title',
		...
		if (isset($color) && !empty($color)) {
			$styles.= '#nz-column-'.$id_counter.' .title {background-color:'.$color.';}';
			$styles.= '#nz-column-'.$id_counter.' .button:hover {
				color: '.$color.';
				box-shadow:inset 0 0 0 2px '.$color.';
			}';

			$styles.= '#nz-column-'.$id_counter.' .pricing:before {
				background-color:'.$color.';
			}';
		}

		$output .='<div id="nz-column-'.$id_counter.'" class="element-animate column highlight-'.sanitize_html_class($high).'">';
			$output .='<div class="column-inner">';
					...
					if (isset($price) && !empty($price)) {
						$output .='<div class="price">';
							if (isset($currency) && !empty($currency)) {
								$output .='<span class="currency">'.esc_html($currency).'</span>';
							}
				...
				if (isset($link) && !empty($link)) {
					...
					$output .='<div class="c-foot">';
						$output .='<a href="'.esc_url($link).'" class="animate-false medium button square button-ghost hover-fill grey_light">'.esc_html($button_text).'</a>';
					$output .='</div>';
				}
	...
	function nz_rposts($atts, $content = null) {
		...
		$recent_posts = new WP_Query(array( 'orderby' => 'date', 'posts_per_page' => absint($posts_number), 'cat' => $cat,'post_status' => 'publish','ignore_sticky_posts' => true));
			if($recent_posts->have_posts()){
				...
				$output .= '<div id="nz-recent-posts-'.$id_counter.'" data-autoplay="'.esc_attr($autoplay).'" data-columns="'.absint($columns).'" class="lazy nz-recent-posts nz-clearfix animate-'.sanitize_html_class($animate).'" data-animate="'.$data_animate.'">';
					...
					while($recent_posts->have_posts()) : $recent_posts->the_post();
						...
						$output .= '<div class="post item format-'.get_post_format().' nz-clearfix" data-grid="ninzio_01">';
							...
							$output .= montserrat_ninzio_thumbnail('Montserrat-Ninzio-Half', $post->ID);
									...
									if ( '' != get_the_title() ){
										if (has_post_format('link')){
											$output .='<h3 class="post-title">';
												$output .='<a href="'.esc_url($nz_link_url).'" title="'.esc_html__("Go to", 'montserrat').' '.$nz_link_url.'" target="_blank">';
													$output .=get_the_title();
												$output .='</a>';
											$output .='</h3>';
										}else{
											$output .='<h3 class="post-title">';
												$output .='<a href="'.get_the_permalink().'" title="'.esc_html__("Read more about", 'montserrat').' '.get_the_title().'" rel="bookmark">';
													$output .=get_the_title();
												$output .='</a>';
											$output .='</h3>';
										}
									}
									$output .='<div class="post-date-full">'.get_the_date("j M Y").'</div>';
									if ('' != get_the_excerpt()){
										$output .='<div class="post-excerpt">'.montserrat_ninzio_excerpt(170).'</div>';
									}
									$output .='<a class="post-more" href="'.get_permalink().'">'.esc_html__("Read more", "montserrat").'<span class="screen-reader-text">'.get_the_title().'</span></a>';
								$output .='</div>';
							$output .='</div>';
				...
				return montserrat_ninzio_not_found('post');
			}
	}
	...
	function nz_rprojects($atts, $content = null) {
		...
		$recent_projects = new WP_Query($recent_query_opt);
			...
			if($recent_projects->have_posts()){
				...
				$output .= '<div id="nz-recent-projects-'.$id_counter.'" class="element-animate '.$layout.' nz-clearfix nz-recent-projects">';
					...
					if ($filter == "true") {
				    	...
				    	if (count($tax_terms) != 0) {
							...
							$output .= '<div class="nz-projects-filter ninzio-filter">';
								$output .= '<div class="container">';
									$output .= '<span class="active filter" data-group="all" data-count="'.$count_posts->publish.'">'.esc_html__('Show All', 'ninzio_addons').'</span>';
									$output .= '<span class="filter-toggle"></span>';
									$output .= '<div class="filter-container">';
										foreach(get_terms('projects-category',$args) as $filter_term) {
											$output .= '<span class="filter" data-group="'.$filter_term->slug.'" data-count="'.$filter_term->count.'">'.$filter_term->name.'</span>';
										}
									$output .= '</div>';
								$output .= '</div>';
							$output .= '</div>';
					...
					$output .= '<div class="recent-projects-wrap nz-clearfix">';
						while($recent_projects->have_posts()) : $recent_projects->the_post();
							...
							$output .= '<div class="projects mix nz-clearfix" data-groups=\'['.implode(', ',$classes).']\' data-grid="ninzio_01">';
								...
								if ($layout == "small-standard" || $layout == "medium-standard" || $layout == "large-standard") {
									...
									if (has_post_thumbnail()){
									    $output .= '<div class="nz-thumbnail">';
									        ...
									        $output .= '<a class="ninzio-overlay" href="'.get_permalink().'"></a>';
								        	$output .= '<div class="project-info-tab">';
								        		$output .= '<a class="project-more" href="'.get_permalink().'"></a>';
								        		if ( '' != get_the_title() ){
													$output .= '<h4 class="project-title">';
														$output .= get_the_title();
													$output .= '</h4>';
			...
			return montserrat_ninzio_not_found('projects');
		}
	}
	...
	function nz_rproducts($atts, $content = null) {
		...
		if (class_exists('Woocommerce')){
			...
			if($recent_products->have_posts()){
					...
					$output .= '<div id="nz-recent-products-'.$id_counter.'" data-columns="'.esc_attr($columns).'" class="lazy woocommerce nz-recent-products nz-clearfix">';
				    	...
				    	if (count($tax_terms) != 0 && $filter == "true") {
							$output .= '<div class="nz-projects-filter nz-products-filter ninzio-filter">';
								$output .= '<div class="container">';
									$output .= '<span class="active filter" data-filter="*" data-count="'.$count_posts->publish.'">'.esc_html__('Show All', 'ninzio_addons').'</span>';
									$output .= '<span class="filter-toggle"></span>';
									$output .= '<div class="filter-container">';
										foreach(get_terms('product_cat',$args) as $filter_term) {
											$output .= '<span class="filter" data-filter=".'.$filter_term->slug.'" data-count="'.$filter_term->count.'">'.$filter_term->name.'</span>';
										}
									$output .= '</div>';
								$output .= '</div>';
							$output .= '</div>';
						...
						$output .= '<ul class="nz-product-posts products">';
							while($recent_products->have_posts()) : $recent_products->the_post();
								...
								$output .= '<li class="mix product element-animate nz-clearfix '.implode(', ',$classes).'" data-grid="ninzio_01">';
									global $product;
				...
				return montserrat_ninzio_not_found('products');
			}
