<?php
	...
	add_action('widgets_init', 'ninzio_addons_register_flickr_widget');
	function ninzio_addons_register_flickr_widget(){
		register_widget( 'Ninzio_Addons_WP_Widget_Photos_From_Flickr' );
	}

	class  Ninzio_Addons_WP_Widget_Photos_From_Flickr extends WP_Widget {

		public function __construct() {
			parent::__construct(
				'photos_from_flickr',
				esc_html__('* Photos from flickr', 'ninzio-addons'),
				array( 'description' => esc_html__('Display photos from flickr', 'ninzio-addons'))
			);
		}
		...
		public function widget( $args, $instance ) {
	 	...
	 	public function form( $instance ) {
	 		...
	 		$defaults = array(
	 			'title'         => esc_html__('Photos from flickr', 'ninzio-addons'),
	 			'photos_number' => '6',
	 			'flickr_id'     => '',
	 		);
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'flickr_id' ); ?>"><?php echo esc_html__( 'Flickr Username:', 'ninzio-addons' ); ?></label> 
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'photos_number' ); ?>"><?php echo esc_html__( 'Number of photos to show:', 'ninzio-addons' ); ?></label> 
