<?php
	...
	add_action('widgets_init', 'ninzio_addons_register_recent_projects_widget');
	function ninzio_addons_register_recent_projects_widget(){
		register_widget( 'Ninzio_Addons_WP_Widget_Recent_Projects' );
	}

	class  Ninzio_Addons_WP_Widget_Recent_Projects extends WP_Widget {

		public function __construct() {
			parent::__construct(
				'recent_projects',
				esc_html__('* Recent Projects', 'ninzio-addons'),
				array( 'description' => esc_html__('Display recent projects with thumbnails', 'ninzio-addons'))
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$output = "";
				...
				if($recent_projects_with_thumbnail->have_posts()){
					...
					echo montserrat_ninzio_not_found('projects');
				}
	 	...
	 	public function form( $instance ) {
			...
			$defaults = array(
	 			'title'        => esc_html__('Recent projects', 'ninzio-addons'),
	 			'posts_number' => '6',
	 			'category'     => ''
	 		);
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'posts_number' ); ?>"><?php echo esc_html__( 'Number of projects to show:', 'ninzio-addons' ); ?></label> 
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php echo esc_html__( 'Enter comma separated list of categories id to filter:', 'ninzio-addons' ); ?></label> 
