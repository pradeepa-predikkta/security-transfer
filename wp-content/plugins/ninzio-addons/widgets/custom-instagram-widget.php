...
class null_instagram_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'null-instagram-feed',
			esc_html__( '* Instagram', 'ninzio-addons' ),
			array(
				'classname' => 'null-instagram-feed',
				'description' => esc_html__( 'Displays your latest Instagram photos', 'ninzio-addons' ),
	...
	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array( 'title' => esc_html__( 'Instagram', 'ninzio-addons' ), 'username' => '', 'size' => 'large', 'link' => esc_html__( 'Follow Me!', 'ninzio-addons' ), 'number' => 9, 'target' => '_self' ) );
	...
	function scrape_instagram( $username ) {
		...
		if ( false === ( $instagram = get_transient( 'instagram-a5-'.sanitize_title_with_dashes( $username ) ) ) ) {
			...
			if ( is_wp_error( $remote ) )
				return new WP_Error( 'site_down', esc_html__( 'Unable to communicate with Instagram.', 'ninzio-addons' ) );

			if ( 200 != wp_remote_retrieve_response_code( $remote ) )
				return new WP_Error( 'invalid_response', esc_html__( 'Instagram did not return a 200.', 'ninzio-addons' ) );
			...
			if ( ! $insta_array )
				return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'ninzio-addons' ) );

			if ( isset( $insta_array['entry_data']['ProfilePage'][0]['user']['media']['nodes'] ) ) {
				$images = $insta_array['entry_data']['ProfilePage'][0]['user']['media']['nodes'];
			} else {
				return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.', 'ninzio-addons' ) );
			}

			if ( ! is_array( $images ) )
				return new WP_Error( 'bad_array', esc_html__( 'Instagram has returned invalid data.', 'ninzio-addons' ) );
			...
			foreach ( $images as $image ) {
				...
				$caption = esc_html__( 'Instagram Image', 'ninzio-addons' );
		...
		if ( ! empty( $instagram ) ) {
			...
			return new WP_Error( 'no_images', esc_html__( 'Instagram did not return any images.', 'ninzio-addons' ) );
