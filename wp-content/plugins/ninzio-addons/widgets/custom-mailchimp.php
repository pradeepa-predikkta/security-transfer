<?php
	...
	add_action('widgets_init', 'ninzio_addons_register_mailchimp_widget');
	function ninzio_addons_register_mailchimp_widget(){
		register_widget( 'Ninzio_Addons_WP_Widget_Mailchimp' );
	}

	class  Ninzio_Addons_WP_Widget_Mailchimp extends WP_Widget {

		public function __construct() {
			parent::__construct(
				'mailchimp',
				esc_html__('* Mailchimp', 'ninzio-addons'),
				array( 'description' => esc_html__('MailChimp Signup Form', 'ninzio-addons'))
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$output .='<div id="mc_embed_signup">';
				...
				$output .='<form action="'.$action.'" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>';
					$output .='<input type="text" value="" name="EMAIL" class="email" id="mce-EMAIL" placeholder="'.esc_html__("Enter email", 'ninzio-addons').'" required>';
				    $output .='<input type="text" name="'.$name.'" tabindex="-1" value="" class="hidden">';
				    $output .='<input type="submit" value="'.esc_html__('Subscribe', 'ninzio-addons').'" name="subscribe" id="mc-embedded-subscribe">';
				$output .='</form>';
	 	...
	 	public function form( $instance ) {
			...
			$defaults = array(
	 			'title'       => esc_html__('Subscribe', 'ninzio-addons'),
	 			'action'      => '',
	 			'name'        => '',
	 			'description' => '',
	 			'subtitle'    => ''
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'subtitle' ); ?>"><?php echo esc_html__( 'Subtitle:', 'ninzio-addons' ); ?></label> 
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'action' ); ?>"><?php echo esc_html__( 'Action:', 'ninzio-addons' ); ?></label> 
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'name' ); ?>"><?php echo esc_html__( 'Name:', 'ninzio-addons' ); ?></label> 
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'description' ); ?>"><?php echo esc_html__( 'Description:', 'ninzio-addons' ); ?></label> 
