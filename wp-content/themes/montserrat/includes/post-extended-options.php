...
add_action("admin_init", "montserrat_ninzio_add_post_meta_box");
function montserrat_ninzio_add_post_meta_box(){

    add_meta_box(
        "ninzio-post-format-options", 
        esc_html__("Post Format Options", 'montserrat'),
        "montserrat_render_ninzio_post_options", 
        "post",
        "normal", 
        "high"
    );
...
function montserrat_render_ninzio_post_options($post) {
    ...
    wp_nonce_field( 'montserrat_ninzio_post_meta_nonce', 'montserrat_ninzio_post_meta_nonce' );
...
?>
    <div id="ninzio-post-featured-media">
        <label for="post-featured-media">
            ...
            <?php echo esc_html__(' - Disable Featured Media in this post (Featured Image / Featured Gallery)', 'montserrat'); ?>
        </label>
    </div>
    <div id="ninzio-post-format-audio" class="ninzio-post-option">
        <h4><?php echo esc_html__("Audio post format options", 'montserrat'); ?></h4>
        <div>
            <label for="audio_mp3"><?php echo esc_html__('Enter  MP3 audio file link here:', 'montserrat'); ?></label>
            <input name="audio_mp3" id="post-audio-mp3" type="text" value="<?php echo esc_url($audio_mp3);?>"/>
        </div>
        <div>
            <label for="audio_ogg"><?php echo esc_html__('Enter  OGG audio file link here:', 'montserrat'); ?></label>
            <input name="audio_ogg" id="post-audio-ogg" type="text" value="<?php echo esc_url($audio_ogg);?>"/>
        </div>
        <div>
            <label for="audio_embed"><?php echo esc_html__('Embed audio here:', 'montserrat'); ?></label>
            <textarea name="audio_embed" id="post-audio-embed"><?php echo esc_url($audio_embed);?></textarea>
        </div>
    </div>
    <div id="ninzio-post-format-video" class="ninzio-post-option">
        <h4><?php echo esc_html__("Video post format options", 'montserrat'); ?></h4>
        <div>
            <label for="video_mp4"><?php echo esc_html__('Enter  MP4 video file link here:', 'montserrat'); ?></label>
            <input name="video_mp4" id="post-video-mp3" type="text" value="<?php echo esc_url($video_mp4);?>"/>
        </div>
        <div>
            <label for="video_ogv"><?php echo esc_html__('Enter  OGV video file link here:', 'montserrat'); ?></label>
            <input name="video_ogv" id="post-video-ogv" type="text" value="<?php echo esc_url($video_ogv);?>"/>
        </div>
        <div>
            <label for="video_webm"><?php echo esc_html__('Enter  WEBM video file link here:', 'montserrat'); ?></label>
            <input name="video_webm" id="post-video-webm" type="text" value="<?php echo esc_url($video_webm);?>"/>
        </div>
        <br>
        <div>
            <div class="ninzio-upload">
                <input name="video_poster" id="post-video-poster" type="hidden" class="ninzio-upload-path" value="<?php echo esc_url($video_poster);?>"/> 
                <input class="ninzio-button-upload button" type="button" value="<?php echo esc_html__('Upload video poster image', 'montserrat') ?>" />
                <input class="ninzio-button-remove button" type="button" value="<?php echo esc_html__('Remove video poster image', 'montserrat') ?>" />
                <img src='<?php echo esc_url($video_poster); ?>' class='nz-img-preview ninzio-preview-upload'/>
            </div>
        </div>

        <div>
            <label for="video_embed"><?php echo esc_html__('Embed video here:', 'montserrat'); ?></label>
            <textarea name="video_embed" id="post-video-embed"><?php echo esc_url($video_embed);?></textarea>
        </div>
    </div>
    <div id="ninzio-post-format-gallery" class="ninzio-post-option">
        <h4><?php echo esc_html__("Gallery post format options", 'montserrat'); ?></h4>
        <div><?php echo esc_html__('Use 2nd/3rd ... Featured Images (in the right sidebar, right after main featured image) to upload images for the gallery post format', 'montserrat'); ?></div>
    </div>
    <div id="ninzio-post-format-link" class="ninzio-post-option">
        <h4><?php echo esc_html__("Link post format options", 'montserrat'); ?></h4>
        <div>
            <label for="link_url"><?php echo esc_html__('Enter link URL here:', 'montserrat'); ?></label>
    ...
    <div id="ninzio-post-format-status" class="ninzio-post-option">
        <h4><?php echo esc_html__("Status post format options", 'montserrat'); ?></h4>
        <div>
            <label for="status_author"><?php echo esc_html__('Enter status author name here:', 'montserrat'); ?></label>
    ...
    <div id="ninzio-post-format-quote" class="ninzio-post-option">
        <h4><?php echo esc_html__("Quote post format options", 'montserrat'); ?></h4>
        <div>
            <label for="quote_author"><?php echo esc_html__('Enter quote author name here:', 'montserrat'); ?></label>
...
add_action( 'save_post', 'montserrat_save_ninzio_post_format_options' );  
function montserrat_save_ninzio_post_format_options( $post_id )  
{  
    
    if( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return; 
    if( !isset( $_POST['montserrat_ninzio_post_meta_nonce'] ) || !wp_verify_nonce( $_POST['montserrat_ninzio_post_meta_nonce'], 'montserrat_ninzio_post_meta_nonce' ) ) return;  
